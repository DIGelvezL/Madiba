...
@NamedQueries({
	...
	@NamedQuery(name="Solicitud.findAudienciasConciliadorFiltroRadicado", query="SELECT DISTINCT s FROM Agenda agd, Audiencia aud, Solicitud s, Designacion d WHERE s.estado != 'GUARDADA' AND s.estado != 'NEGADA' AND agd.audiencia.idAudiencia = aud.idAudiencia AND aud.solicitud.idSolicitud = s.idSolicitud AND s.nroRadicado like :nroRadicado AND s.idSolicitud = d.solicitud.idSolicitud AND d.aceptada <> false AND d.conciliador.idConciliador = :idConciliador AND s.estado like 'AUDIENCIA%' ORDER BY s.fecha"),
	
	@NamedQuery(name="Solicitud.findSolicitudes", query="SELECT DISTINCT s FROM Solicitud s WHERE s.estado != 'GUARDADA' AND s.estado != 'NEGADA' AND s.fecha between :fechaInicial AND :fechaFinal ORDER BY s.fecha"),
	@NamedQuery(name="Solicitud.findSolicitudesFiltroParteFecha", query="SELECT DISTINCT s FROM Solicitud s, Parte p WHERE s.estado != 'GUARDADA' AND s.estado != 'NEGADA' AND s.fecha between :fechaInicial AND :fechaFinal AND s.idSolicitud = p.solicitud.idSolicitud AND p.identificacion = :identificacion AND p.tipoParte=:tipoParte ORDER BY s.fecha"),
	@NamedQuery(name="Solicitud.findSolicitudesFiltroConciliadorFecha", query="SELECT DISTINCT s FROM Solicitud s, Designacion d, Conciliador c WHERE s.estado != 'GUARDADA' AND s.estado != 'NEGADA' AND s.fecha BETWEEN :fechaInicial AND :fechaFinal AND s.idSolicitud = d.solicitud.idSolicitud AND d.aceptada <> false AND d.conciliador.idConciliador = c.idConciliador AND c.identificacion = :identificacion ORDER BY s.fecha"),
	...
	@NamedQuery(name="Solicitud.findSolicitudesFiltroRadicado", query="SELECT DISTINCT s FROM Solicitud s WHERE s.estado != 'GUARDADA' AND s.estado != 'NEGADA' AND s.nroRadicado like :nroRadicado ORDER BY s.fecha")
